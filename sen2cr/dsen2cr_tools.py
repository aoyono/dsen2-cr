...
from sen2cr.tools.dataIO import make_dir, DataGenerator, process_predicted
from sen2cr.tools.myCallbacks import CSV_NBatchLogger, NBatchLogger, TensorBoardWrapper
...
def train_dsen2cr(
    ...
    model_filepath = os.path.join(
        out_path_train, model_name + "_{epoch:02d}-{val_loss:.4f}" + ".hdf5"
    )
    ...
    csv_batch_logger = CSV_NBatchLogger(
        1, out_path_train, model_name, initial_epoch, separator=";"
    )

    # instantiate NBatch logger
    batch_logger = NBatchLogger(
        log_step_freq, out_path_train, model_name, initial_epoch, lr
    )
    ...
    tensorboard = TensorBoardWrapper(
        val_tensorboard_generator,
        input_dim=input_shape,
        nb_steps=1,
        batch_size=batch_size,
        log_dir=out_path_train,
        histogram_freq=1,
        write_graph=False,
        batch_nr=batch_size,
        write_grads=True,
